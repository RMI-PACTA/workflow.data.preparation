# Sys.setenv(R_CONFIG_ACTIVE = "desktop")
logger::log_threshold(Sys.getenv("LOG_LEVEL", ifelse(interactive(), "INFO", "INFO")))
...
logger::log_info("Formatting and saving ISS emission intensities data.")
  ...
  func = function(
    ...
    iss_company_emissions <-
      pacta.data.preparation::prepare_iss_company_emissions(
        factset_iss_emissions_data = readRDS(factset_iss_emissions_data_path)
      )
    ...
    iss_entity_emission_intensities_output_path <-
      file.path(data_prep_outputs_path, "iss_entity_emission_intensities.rds")

    saveRDS(
      object = pacta.data.preparation::prepare_iss_entity_emission_intensities(
      ...
      file = iss_entity_emission_intensities_output_path
    )

    iss_average_sector_emission_intensities_output_path <-
      file.path(data_prep_outputs_path, "iss_average_sector_emission_intensities.rds")

    saveRDS(
      object = pacta.data.preparation::prepare_iss_average_sector_emission_intensities(
        iss_company_emissions = iss_company_emissions,
        factset_financial_data = factset_financial_data,
        factset_entity_info = factset_entity_info,
        factset_entity_financing_data = factset_entity_financing_data,
      ...
      file = iss_average_sector_emission_intensities_output_path
    )
...
logger::log_info("ISS emission intensities data prepared.")
...
invisible(callr::r(
  func = function(config,
    ...
    ar_company_id__factset_entity_id_path <-
      file.path(
        config[["asset_impact_data_path"]],
        config[["ar_company_id__factset_entity_id_filename"]]
      )
    factset_entity_info_path <-
      file.path(
    ...
    masterdata_ownership_path <-
      file.path(
        config[["asset_impact_data_path"]],
        config[["masterdata_ownership_filename"]]
      )
    masterdata_debt_path <-
      file.path(
        config[["asset_impact_data_path"]],
        config[["masterdata_debt_filename"]]
      )

    factset_entity_id__ar_company_id <-
      pacta.data.preparation::prepare_factset_entity_id__ar_company_id(
        ar_company_id__factset_entity_id = readr::read_csv(
          file = ar_company_id__factset_entity_id_path,
          col_types = "c",
          show_col_types = FALSE
        )
      )
    ...
    entity_info <-
      pacta.data.preparation::prepare_entity_info(
        data = readRDS(file = factset_entity_info_path),
        factset_entity_id__ar_company_id = factset_entity_id__ar_company_id,
        factset_industry_map_bridge = factset_industry_map_bridge,
        factset_manual_pacta_sector_override = factset_manual_pacta_sector_override
      )

    ar_company_id__country_of_domicile <-
      pacta.data.preparation::prepare_ar_company_id__country_of_domicile(
        entity_info = entity_info
      )

    ar_company_id__credit_parent_ar_company_id <-
      pacta.data.preparation::prepare_ar_company_id__credit_parent_ar_company_id(
        entity_info = entity_info
      )
    ...
    masterdata_ownership_datastore <-
      pacta.data.preparation::prepare_masterdata(
        data = readr::read_csv(
          file = masterdata_ownership_path,
          na = "",
          show_col_types = FALSE
        ),
    ...
    pacta.data.validation::validate_masterdata_ownership_datastore(
      data = masterdata_ownership_datastore
    )

    saveRDS(
      object = masterdata_ownership_datastore,
      file = file.path(data_prep_outputs_path, "masterdata_ownership_datastore.rds")
    )

    masterdata_debt_datastore <-
      pacta.data.preparation::prepare_masterdata_debt(
        masterdata_debt_raw = readr::read_csv(
          file = masterdata_debt_path,
          na = "",
          show_col_types = FALSE
        ),
        ar_company_id__country_of_domicile = ar_company_id__country_of_domicile,
    ...
    pacta.data.validation::validate_masterdata_debt_datastore(
      data = masterdata_debt_datastore
    )

    saveRDS(
      object = masterdata_debt_datastore,
      file = file.path(data_prep_outputs_path, "masterdata_debt_datastore.rds")
    )
  },
  args = list(
    config,
  ...
  callr::r(
    func = function(config, masterdata_ownership_datastore, scenarios_long_source, relevant_years, scenario_regions, index_regions, filename) {
      saveRDS(
        ...
        file = file.path(config[["data_prep_outputs_path"]], filename)
      )
    },
...
# load large files once in the beginning ----------------------------------

logger::log_info("Loading large data files.")

financial_data <-
  pacta.data.preparation::prepare_financial_data(
    data = readRDS(factset_financial_data_path),
    issue_code_bridge = factset_issue_code_bridge
  )
...
factset_entity_id__security_mapped_sector <-
  pacta.data.preparation::prepare_factset_entity_id__security_mapped_sector(
    entity_info = entity_info
  )

factset_entity_id__credit_parent_id <-
  pacta.data.preparation::prepare_factset_entity_id__credit_parent_id(
    entity_info = entity_info
  )
...
ar_company_id__sectors_with_assets__ownership <-
  pacta.data.preparation::prepare_ar_company_id__sectors_with_assets__ownership(
    masterdata_ownership_datastore = masterdata_ownership_datastore,
    relevant_years = relevant_years
  )
...
abcd_flags_equity <-
  pacta.data.preparation::prepare_abcd_flags_equity(
    ...
    factset_entity_id__ar_company_id = factset_entity_id__ar_company_id,
    factset_entity_id__security_mapped_sector = factset_entity_id__security_mapped_sector,
    ar_company_id__sectors_with_assets__ownership = ar_company_id__sectors_with_assets__ownership
  )
...
ar_company_id__sectors_with_assets__debt <-
  pacta.data.preparation::prepare_ar_company_id__sectors_with_assets__debt(
    masterdata_debt_datastore = masterdata_debt_datastore,
    relevant_years = relevant_years
  )
...
abcd_flags_bonds <-
  pacta.data.preparation::prepare_abcd_flags_bonds(
    ...
    factset_entity_id__ar_company_id = factset_entity_id__ar_company_id,
    factset_entity_id__security_mapped_sector = factset_entity_id__security_mapped_sector,
    ar_company_id__sectors_with_assets__debt = ar_company_id__sectors_with_assets__debt,
    factset_entity_id__credit_parent_id = factset_entity_id__credit_parent_id
  )
...
saveRDS(
  object = fund_data,
  file = file.path(config[["data_prep_outputs_path"]], "fund_data.rds")
)
...
saveRDS(
  ...
  file = file.path(config[["data_prep_outputs_path"]], "total_fund_list.rds")
)
...
saveRDS(
  ...
  file = file.path(config[["data_prep_outputs_path"]], "isin_to_fund_table.rds")
)
